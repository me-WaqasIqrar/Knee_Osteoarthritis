    ...
    for cls_name in sorted(os.listdir(path)):
        ...
        for img_name in os.listdir(cls_path):
            if img_name.endswith((".png", ".jpg", ".jpeg")):
                images_path.append(os.path.join(cls_path, img_name))
                label.append(int(cls_name))
                save_dir.append(os.path.join(RESULT_SAVE_PATH,cls_name,img_name))
    return images_path,label,save_dir
...
def main_inference(model, processor, IMG_PATH, save_dir,label):
    ...
    if OUT_FILENAME is None:
        ...
        if not os.path.exists(os.path.join(RESULT_SAVE_PATH,str(label))):
            os.makedirs(os.path.join(RESULT_SAVE_PATH,str(label)), exist_ok=True)
...
if __name__ == '__main__':
    ...
    for i, img_path in enumerate(images_path):
        ...
        main_inference(model, processor, img_path, save_dir[i], label[i])
